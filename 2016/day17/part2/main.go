...
func findLongestPath(passcode string) int {
    ...
    for len(queue) > 0 {
        ...
        if current.position == end {
            if len(current.path) > longestPathLength {
                longestPathLength = len(current.path)
            }
            continue
        }
...
func main() {
    passcode := "vwbaicqe"
    longestPathLength := findLongestPath(passcode)
    fmt.Println("Longest path length:", longestPathLength)
}
