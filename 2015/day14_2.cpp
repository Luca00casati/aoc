...
int main() {
    ...
    vector<Raindear> raindears = get_raindears("test.txt");
    ...
    auto maxScoreReindeer = std::max_element(
        raindears.begin(), raindears.end(),
        [](const Raindear &a, const Raindear &b) {
            return a.points < b.points;
        });

    if (maxScoreReindeer != raindears.end()) {
        std::cout << "Winner: " << maxScoreReindeer->name << " with " << maxScoreReindeer->points << " points.\n";
    } else {
        std::cout << "No reindeer found.\n";
    }
